...
生活页使用 [一言 API](https://hitokoto.cn/usage) 获取文学、诗词和哲思分类的随机句子。接口地址为：

```text
https://v1.hitokoto.cn/?c=d&c=i&c=k&encode=json&max_length=55
```

页面会在当前会话中缓存最近一次结果，减少重复请求；每次点击“换一句”才主动获取新内容。如果接口超时或不可用，会显示仓库内置的公共领域经典句子，因此不会留下空白卡片。随机句子正文和出处均通过 `textContent` 写入，避免将接口内容当作 HTML 执行。
