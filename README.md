...
- 生活页内置三种由 Web Audio API 实时生成的氛围音乐；
...
| 生活 | `life.html` | `en/life.html` | 近期状态、双语每日哲思与氛围音乐播放器 |
...
氛围音乐播放器基于浏览器原生 Web Audio API，提供“深度专注”“夜间阅读”和“安静清晨”三种模式。声音由低频振荡器、低通滤波和缓慢音高漂移实时合成，支持播放、暂停、上一种、下一种、直接选择模式、音量调节和本次播放计时。播放器遵循浏览器自动播放策略，必须由用户主动点击后才会发声；它不下载外部歌曲、不依赖流媒体 API，也不上传播放数据，因此适合 GitHub Pages 的纯静态部署。

如果浏览器不支持 Web Audio，页面会保留播放器界面并给出兼容性提示。系统开启“减少动态效果”时，唱片旋转与音量柱动画会自动停用，但声音控制仍可使用。
